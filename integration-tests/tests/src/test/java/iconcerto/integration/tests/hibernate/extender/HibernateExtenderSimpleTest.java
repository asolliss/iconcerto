...
public class HibernateExtenderSimpleTest extends AbstractIConcertoDatabaseIntegrationTest {
	...
	public void testGetTransactionManager() {
		ServiceReference<PlatformTransactionManager> transactionManagerServiceReference =
				bundleContext.getServiceReference(PlatformTransactionManager.class);
	...
	public void testStartingValidBundleForExtendingBeforeStartingHibernateExtender() throws BundleException, IOException {
		...
		try {			
			...
			try {
				...
				waitOnContextCreation(hibernateExtenderBundle.getSymbolicName());
